    ...
    $.post("/menu/query_filter_dish",
        ...
        function(data, status){
            ...
            for (let d in dishes){
                ...
                dish_html = dish_html.replace("dish_gradients", JSON.parse(dish.fields.ingredients).join(", "))
...
const click_on_confirm= ()=>{
    ...
    $.post("/menu/create_query", 
        ...
        function(data, status){
            if (status == 'success'){
                toastr.options.onHidden = function() { window.location = MENU_INDEX }
                toastr.success("Tạo menu thành công, đang chuyển trang...", "Hệ thống")
            }
        }
    )
}
